...
\begin{solution}
	...
	\begin{enumerate}
		...
		\end{align*}
		当超平面与 $S$ 有切点时取到等号。
		\item 要求 $\max\inf_{\mathbf{x}\in X}\,L(\mathbf{x},\lambda^*,\mu^*)$，由于 $\mathbf{x} \in X$，所以有 $\mathbf{g}(\mathbf{x}) \leqslant 0, \mathbf{h}(\mathbf{x}) = 0$，则有
		\begin{align*}
		...
		\end{align*}
		因为 $\lambda^* \geqslant 0$，所以 $\lambda \mathbf{g}(\mathbf{x}) \leqslant 0$，所以如果要使得上式最大，那么只能 $\lambda \mathbf{g}(\mathbf{x}) = 0$，则
		\begin{align*}
			&\inf_{\mathbf{x}\in X}\,\{f(\mathbf{x})+\lambda \mathbf{g}(\mathbf{x})\} \\
